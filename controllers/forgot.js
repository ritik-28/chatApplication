  ...
  try {
    ...
    if (user) {
      ...
      const doneEmail = await tranEmailApi.sendTransacEmail({
        sender,
        to: recievers,
        subject: `Password reset link for Tiwachat`,
        htmlContent: `<h4>visit this link</h4><a href= "http://34.238.254.129:3000/password/resetpassword/${id}">Reset Password</a>`,
      });
  ...
  try {
    ...
    if (gotit) {
      ...
      res.status(200).send(`<html>
                                <form action="http://34.238.254.129:3000/password/updatepassword/${id}" method="get">
